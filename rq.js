...
// and a callback that will take the next step.
                    ...
                    let next_requestor = requestors[index];
                    soon(function () {
                        let once = true;
                        if (typeof callback === "function") {
...
// property names.
        ...
        if (typeof requireds === "object" && !Array.isArray(requireds)) {
            ...
            if (optionals) {
                ...
                let optional_names = Object.keys(optionals);
        ...
        return function requestor(callback, initial) {
            let cancels = [];
            let optionals_remaining;
            let optionals_successes = 0;
            let requireds_length = requireds.length;
            let requireds_remaining = requireds.length;
            let results = [];
            let timeout_until;
            let timeout_id;
...
// expires.
        ...
        return function requestor(callback, initial) {
            let cancels = [];
            let remaining = requestors.length;
            let timeout_id;
